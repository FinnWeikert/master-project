...
def loso_correlation_analysis(
    ...
    for surgeon_out in tqdm(unique_surgeons, desc="Analyzing Folds"):
        ...
        for feat in candidate_features:
            r, _ = spearmanr(df_train[feat], df_train[target])
            fold_results.append({
...
def permutation_significance_test(
    ...
    top_n=1,
    show_plot=True,
    metric_name="Partial R2"
):
    """
    Performs a leakage-free permutation test to evaluate the significance 
    of a real observed metric against a null distribution.
    """
    ...
    for seed in tqdm(range(n_shuffles)):
        ...
        # Extract metrics
        null_distribution.append(df_res['Shuffled_R2_mean'].values[0])
        
        # Safe extraction for velocity_corr in case it wasn't selected in a shuffle
        vel_row = df_res[df_res['Feature'] == 'velocity_corr']
        if not vel_row.empty:
            vel_corr_null.append(vel_row['Shuffled_R2_mean'].values[0])
            
        stability_null.append(df_res['Selection_Stability'].values[0])

    # --- Statistical Calculations ---
    null_array = np.array(null_distribution)
    p_value = np.mean(null_array >= real_val)
    z_score = (real_val - np.mean(null_array)) / np.std(null_array)

    print(f"\n--- Significance Results ---")
    print(f"Real {metric_name}: {real_val:.4f}")
    ...
    if show_plot:
        ...
        plt.hist(null_array, bins=20, alpha=0.6, color='skyblue', edgecolor='black', 
                 label=f'Null Distribution (n={n_shuffles})')
        plt.axvline(real_val, color='red', linestyle='--', linewidth=2, 
                    label=f'Real Signal ({metric_name}={real_val:.3f})')
        
        plt.title(f"Permutation Test: {metric_name} vs. Random Noise")
